...
import {
    ...
    MapPin, Mail, Phone, Edit, Trash2, Eye, Award
} from 'lucide-react';
...
const PlacementOfficerDashboard = () => {
    ...
    const [students, setStudents] = useState(() => {
        ...
        // Convert to array format with default values
        return Object.entries(allStudents).map(([studentEmail, studentData]) => ({
            id: Math.random(),
            name: studentData.name || 'Unknown',
            email: studentEmail,
            branch: studentData.college || 'CS',
            ...
            { id: 1, name: 'Rahul Sharma', email: 'rahul@example.com', branch: 'CS', cgpa: '8.9', status: 'Placed', company: 'Google', package: '₹22 LPA', resume: 'rahul_sharma_resume.pdf' },
            { id: 2, name: 'Priya Patel', email: 'priya@example.com', branch: 'IT', cgpa: '9.1', status: 'Placed', company: 'Microsoft', package: '₹18 LPA', resume: 'priya_patel_resume.pdf' }
        ]);
    });
    ...
    const stats = [
        { label: 'Total Students', value: students.length.toString(), icon: Users, color: '#6366f1' },
        { label: 'Placed Students', value: students.filter(s => s.status === 'Placed').length.toString(), icon: CheckCircle, color: '#10b981' },
        { label: 'Active Companies', value: '45', icon: Building2, color: '#f59e0b' },
        { label: 'Placement Rate', value: (((students.filter(s => s.status === 'Placed').length / students.length) * 100).toFixed(1)) + '%', icon: TrendingUp, color: '#ec4899' }
    ];
    ...
    const navItems = [
        ...
        { key: 'internships', icon: Award, label: 'Internship Tracking', path: '/officer-internship-tracking' },
            ...
            <div className="glass" style={{ width: '280px', margin: '1rem', borderRight: '1px solid var(--glass-border)', display: 'flex', flexDirection: 'column', padding: '2rem' }}>
                ...
                <nav style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem', flex: 1 }}>
                    {navItems.map(({ key, icon: Icon, label, path }) => (
                        <div
                            key={key}
                            onClick={() => path ? navigate(path) : setActiveTab(key)}
                ...
                {activeTab === 'students' && (
                    <div className="glass" style={{ padding: '2rem' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 1.5fr', gap: '1rem', padding: '0.8rem 1.2rem', color: 'var(--text-gray)', fontSize: '0.85rem', fontWeight: 600 }}>
                            ...
                            {students.map((student) => {
                                ...
                                return (
                                    <div key={student.id} className="glass-card" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 1.5fr', gap: '1rem', padding: '1rem 1.2rem', alignItems: 'center' }}>
                                        ...
                                        <div style={{ display: 'flex', gap: '0.5rem' }}>
                                            <button 
                                                ...
                                                style={{ padding: '0.4rem 0.8rem', borderRadius: '6px', fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '0.3rem' }}
                                                title="View Resume"
                                            >
